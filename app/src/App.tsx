...
const REGIME_ID = 'ro-draft-2026-07-16';
...
export default function App() {
  ...
  useEffect(() => {
    const url = `${import.meta.env.BASE_URL}data/regimes/${REGIME_ID}.json`;
    fetch(url)
  ...
  const metrics = useMemo(() => (regime ? structure(regime) : null), [regime]);

  if (error) return <div className="wrap"><p className="loading">Nu s-au putut încărca datele: {error}</p></div>;
  if (!regime || !metrics) return <div className="wrap"><p className="loading">Se încarcă grila…</p></div>;
  ...
  return (
    <div className="wrap">
      ...
      <footer>
        Sursă: proiectul de lege MMFTSS din 16.07.2026 și anexele de coeficienți. Fiecare număr
        din <code>data/</code> poartă documentul și articolul sau celula din care provine.{' '}
